...
from .tool_patch import check_is_image
...
class Dataset_Return_Four(Dataset):
    ...
    def __getitem__(self, i):
        ...
        image_blue_LL_norm = np.uint8(cv2.resize(image_blue_LL_norm,(256,256),interpolation=cv2.INTER_CUBIC))
        image_green_LL_norm = np.uint8(cv2.resize(image_green_LL_norm,(256,256),interpolation=cv2.INTER_CUBIC))
        image_red_LL_norm = np.uint8(cv2.resize(image_red_LL_norm,(256,256),interpolation=cv2.INTER_CUBIC))
