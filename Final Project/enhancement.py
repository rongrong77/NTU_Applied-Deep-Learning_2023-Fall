...
from base.method import Dataset_Return_Four
...
def unet_train(epochs, gpu, base_model_name, encoder_weights, generator_lr, discriminator_lr, lambda_bce, threshold,
	...
	weight_path = './Unet/stage2_dibco_' + base_model_name + '_' + str(int(lambda_bce)) + '_' + str(generator_lr) + '_' + str(threshold) + '_' + str(focal_gamma) 
	print(weight_path)
		...
		for idx, (images, masks) in enumerate(train_loader):
			...
			for channel in range(4):
				...
				if focal_gamma == 0:
					...
					bce_loss = sigmoid_focal_loss(masks_pred, masks[channel], alpha = .25, gamma = focal_gamma, reduction = 'mean')
