...
def main():
    ...
    # Load pretrained model and tokenizer
      # Initialize data_files as an empty dictionary
    if data_args.train_file is not None or data_args.validation_file is not None or data_args.test_file is not None:
        ...
        if data_args.train_file is not None:
            ...
            data_files["train"] = data_args.train_file  # Add the file path
            extension = data_args.train_file.split(".")[-1]

        if data_args.validation_file is not None:
            with open(data_args.validation_file, 'r', encoding='utf-8') as f:
                validation_data = json.load(f)
            data_files["validation"] = data_args.validation_file  # Add the file path
            extension = data_args.validation_file.split(".")[-1]

        if data_args.test_file is not None:
            with open(data_args.test_file, 'r', encoding='utf-8') as f:
                test_data = json.load(f)
            data_files["test"] = data_args.test_file  # Add the file path
    ...
    def preprocess_train_function(examples):
        idx = examples[paragraphs_idx_name]
        with open("debug_output.txt", "a", encoding="utf-8") as debug_file:
            debug_file.write("context_json: {}\n".format(context_json))
            debug_file.write("idx: {}\n".format(idx))
        first_sentences = [[question] * 7 for question in examples[question_name]]
        paragraphs_idx = [idx + random.sample(set(range(len(context_json))) - set(idx), max(0, 7 - len(idx))) if len(idx) < 7 else idx for idx in examples[paragraphs_idx_name]]
        second_sentences = [
